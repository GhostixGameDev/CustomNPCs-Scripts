...
var HP=800000
var HPRegen=5000
var startPoint=[1812,66,1218] //Change this to your npc starting point
...
function makeClones(npc){
    for(var i=0; i<6; i++){
        ...
        npc.getWorld().spawnClone(npc.getPos(), workingTab, clone)
    }
}
//Critics
...
function init(t){
    ...
    t.npc.setTitle("§2Artista Marcial§6")
    //Shield and Skill
...
function killed(t){
    t.npc.say("Q-Que?");
    t.npc.getWorld().spawnClone(t.npc.getPos(), workingTab, transformationName)
    t.npc.despawn()
}
